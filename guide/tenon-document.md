...
A Flow has one `source` instance, a `process.script` containing Lua source, and a nonempty `sinks` list. Source and Sink bindings must implement those interfaces. A dual-interface instance can be both sides of the same Flow. Each Source instance may be bound by only one Flow. No instance may remain completely unreferenced. Every interface declared by a Program must be bound; a dual-interface Program cannot be used only as Source or only as Sink. A Sink may be shared by multiple Flows. Document, Instance and Flow ids are 1–128 UTF-8 bytes without C0/C1 control characters; object keys supply Instance/Flow ids, without another id field in their values. See the [quickstart](quickstart.md) for a complete executable example.
